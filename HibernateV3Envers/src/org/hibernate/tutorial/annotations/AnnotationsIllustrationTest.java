...
public class AnnotationsIllustrationTest extends TestCase {
	...
	public void testBasicUsage() {
		...
		session = sessionFactory.openSession();
		session.beginTransaction();
		Event myEvent = (Event) session.get(Event.class, 2L); 
		...
		session = sessionFactory.openSession();
		session.beginTransaction();
		myEvent = (Event) session.get(Event.class, 2L);
//		assertEquals("A follow up event (rescheduled)", myEvent.getTitle());
		...
		List revisions =  reader.getRevisions(Event.class, 2L);
		System.out.println("revisions: " + revisions);
		Collections.sort(revisions);
		int latestRev = (int)revisions.get(revisions.size()-1);
		Event secondRevision = reader.find(Event.class, 2L, latestRev);
